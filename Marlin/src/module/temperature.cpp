 ...
 */
  ...
  // Sanity-check max readable temperatures (el -25 lo puse para poder ponerle un maxtemp de 300 grados)
  #define CHECK_MAXTEMP_(N,M,S) static_assert( \
    S >= 998 || M - 25 <= _MAX(TT_NAME(S)[0].celsius, TT_NAME(S)[COUNT(TT_NAME(S)) - 1].celsius) - HOTEND_OVERSHOOT , \
